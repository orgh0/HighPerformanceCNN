...
void rmsprop_update(Container *square_grads, Container *weights,
                    const Container *grads, float learning_rate, float l2,
                    float beta)
{
...
void RMSProp::regist(std::vector<std::pair<Container *, Container *>> params)
{
    for (auto iter = params.begin(); iter != params.end(); iter++)
    {
        this->parameter_list.push_back(iter->first);
        this->grad_list.push_back(iter->second);
        this->square_grad.emplace_back(std::make_unique<Container>(
            iter->first->get_shape(), SQUARE_GRD));
    }
}
