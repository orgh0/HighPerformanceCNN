...
Container::Container(const std::vector<int> &_shape): shape(_shape) {
    ...
    this->data.resize(size) //Flattern the container
}
...
Container::Container(const std::vector<int> &_shape, const std::vector<float> &_data) : shape(_shape), data(_data.begin(), _data.end()) {
