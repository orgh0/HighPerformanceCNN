...
__global__ void log_softmax_h(const float *input1, float *output,
...
void log_softmax(const Container *input1, int dim, Container *outputs)
{
    ...
    log_softmax_h<<<grid_size, BLOCK_SIZE, shared_memory_size>>>(
...
__global__ void d_log_softmax_h(const float *output_grads,
                                         const float *input1,
                            ...
                            int dim, Container *inputs_grad)
{
    ...
    d_log_softmax_h<<<grid_size, BLOCK_SIZE, shared_memory_size>>>(
...
void LogSoftmax::forward()
{
    ...
    log_softmax(input, this->dim, this->output.get());
}
...
void LogSoftmax::backward()
{
    ...
    d_log_softmax(output_grad, input, this->dim, this->grad.get());
}